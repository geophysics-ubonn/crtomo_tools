...
def main():
    data = open_data()
    inv = open_inv()
    if data[:, 0].all() - inv[:, 0].all() == 0 and data[:,
           1].all() - inv[:, 1].all() == 0:
        ...
        for AB, MN, rho, pha in data:
            ...
            receiver.append(r)
        mag_res = np.column_stack((source,
                       receiver,
                       data[:, 2] - inv[:, 2],))
        
        vmin = min(mag_res[:, 2])
        vmax = max(mag_res[:, 2])
        shrunk_cmap = shiftedColorMap(mpl.cm.coolwarm,
        ...
        pha_res = np.column_stack((source,
                       receiver,
                       data[:, 3] - inv[:, 3],))
